...
`PurePosixPath` 和 `PureWindowsPath` 分别表示两种路径风格，前者是 [[POSIX]] 系统接口的路径风格，后者是以 [[Windows]] 系统为代表的路径风格。
...
由于 POSIX 没有盘符，因此 POSIX 路径只要包含根目录就是绝对路径，而 Windows 系统中的路径必须同时包含盘符和根目录才算。

因此对于 Windows 来说，存在一类既不是绝对路径，也不是相对路径的路径对象；即没有盘符的目录路径对象。
...
```Python
>>> PureWindowsPath("c:/Windows", "e:Files.e")
PureWindowsPath('e:Files.e')  # e:Files.e 可以生成路径对象，但是该对象代表的路径本身是无效的
...
因此，不能直接使用 pathlib 模块像处理文件一样处理带有协议头的网址。
...
PurePosixPath('LinkDir/../SubDir')  # 软链接，实际能否访问，纯路径对象不考虑
...
`str()` 返回的是 [[Unicode]] 字符串，这在 Windows 下是规范表示法，但 POSIX 需要考虑其编码方式，有时也会需要用 `bytes()` 方法得到字节表示。

### 排序和比较

相同风格的路径之间可以进行排序和比较。
它们的比较，相当于是路径各部分构成的字符串数组进行比较。
...
此外，路径对象是可哈希的。
和比较一样，哈希也是对于路径各部分构成的字符串数组而言的，所以也会有路径风格的差异。
...
返回的是各个部分组合而成的元祖，包括盘符和根路径（如果有的话）。
...
返回的也是字符串。Windows 系统也有根目录，与盘符是无关的。
...
`parents` 返回了一个序列，使用从 0 开始的下标，可以从近到远依次访问所有的父路径对象。

这是纯路径计算，因此不会涉及到软链接的转换。
...
>>> p = PureWindowsPath("/usr/lib/python3")
>>> list(p.parents)
[PureWindowsPath('/usr/lib'), PureWindowsPath('/usr'), PureWindowsPath('/')]
```

### name 文件名

`name` 属性指向 `parts` 除盘符和根目录外的最后一部分。
...
`suffix` 属性指向 `name` 属性的最后一个后缀，它包含标识后缀的那个点。
但是只有点时，只会返回空字符串。
...
(.gitignore', '')  # name 存在，但 suffix 不存在
```
...
`stem` 是无最后一个后缀的 `name`，相当于 `name - suffix`。
...
允许传入多个参数，只有当所有参数满足相对路径的关系时，才会返回 `True`。

```Python
>>> PurePosixPath("/etc").is_relative_to("/", "/etc")
True

>>> PurePosixPath("/etc").is_relative_to("/", "/usr")
False
```

### is_reserved()

Windwos 系统会预设部分路径作为保留路径，POSIX 则不存在，总是返回 `False`。
...
将传入的多个参数和调用该方法的路径对象拼接在一起。
和初始化参数时一致，该方法也会遵循诸如根目录取代等规则。
...
绝对路径和相对路径之间也是不可计算的，即使它们可能在真实文件系统上是可计算的。
...
*pattern* 是相对的，那么路径对象既可以是绝对路径也可以是相对路径，匹配从右向左匹配；*pattern* 是绝对的，那么路径对象必须是绝对路径，且需要从左向右完全匹配。
