...
但当 `obj.foo` 被赋值给了全局的 `foo` 变量后，仅对内存地址进行了赋值，全局的 `foo` 会越过 `obj` 直接指向函数主体。
在调用 `foo()` 时，`obj` 已被越过，因此不存在直接调用的对象，故而指向默认的 `global` 全局环境。
...
[[JavaScript]] 中的 [[词法环境]] 有两种：全局环境 `global` 和若干个由函数定义而来的函数环境。

同级的函数环境之间彼此不可见。
子函数允许引用父函数的变量，但当子函数中存在和父函数同名变量时，子函数会进行覆盖。

`this` 也遵循这一规则，每个函数都有各自的 `this` 变量，彼此在各自的词法环境中独立；即使是子函数也不会突破自己的词法环境去调用父函数的 `this`。

有如下代码，`obj` 对象构造了“对象-函数-函数”的三层嵌套。
最内层函数因为不存在调用它的对象，从而返回指向了默认的全局环境的 `this`，最后返回 4。
...
由于 [[类]] 的 [[实例]] 也是对象，因此类方法和其他普通函数一样，`this` 不总是跟着实例走，而是取决于如何被调用的。
...
根据词法环境的规则，箭头函数的 `this` 引用自它的父函数，也就是说 `this` 依赖于父函数的环境。
...
将箭头函数以对象的 [[字面量]] 形式创建，由于对象不创建函数含经，故而 `this` 不会指向这个对象，而是会根据词法环境规则被设置。
