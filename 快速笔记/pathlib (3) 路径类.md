...
但是手动实例化的话，只能使用与当前系统风格相同的类，错误使用的话会抛出 `NotImplementedError`。

## 类方法

### cwd()

返回一个表示当前目录的 [[绝对路径]] 的对象。
...
返回迭代器，迭代产生路径下的所有路径对象，顺序是随机的，不包括 `.` 和 `..`。
...
将给定路径创建文件，如果给出 `mode`，则和当前进程的 `unmask` 值合并确定模式。

如果文件已存在，`exist_ok=True` 会使得函数成功；否则，抛出 `FileExistsError`。
